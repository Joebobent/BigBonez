jQuery(document).ready(function($) {
	...
	jQuery.fn.upload_header_background = function( button ) {
		...
		file_frame.on( 'select', function() {
		  ...
		  jQuery("#bnz-header img").attr('src',attachment.url);
		  jQuery( '#bnz-header img' ).show();
	...
	jQuery('#bnz-header').on( 'click', '#upload_header_background_button', function( event ) {
		event.preventDefault();
		jQuery.fn.upload_header_background( jQuery(this) );
	});

	jQuery('#bnz-header').on( 'click', '#remove_header_background_button', function( event ) {
		event.preventDefault();
		jQuery( '#upload_header_background' ).val( '' );
		jQuery( '#bnz-header img' ).attr( 'src', '' );
		jQuery( '#bnz-header img' ).hide();
